...
export async function cancelNewItemIfEmpty(
  filename: string,
  ...
  // custom dialog for cancelling a newly-created empty item
  const prompt = dialog.showMessageBoxSync(mainWindow, {
    title: 'Discard new item?',
    message: `The item "${filename}" appears to be empty (no layers). Do you want to delete it?`,
    buttons: ['Delete', 'Keep'],
    defaultId: 0,
    cancelId: 1,
  });
